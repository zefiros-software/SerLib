...
namespace
{
    ...
    public:
        ...
        TestClass1( uint32_t seed = 233232 )
        {
            ...
            for ( uint32_t i=0; i < its; ++i )
            {
                mMember[i] = ( ( (T)rand() / RAND_MAX ) * std::numeric_limits< T >::max() );
            }
        }
    ...
    public:
        ...
        TestClass2( uint32_t seed = 233232 )
        {
            ...
            for ( uint32_t i=0; i < its; ++i )
            {
                mMemberT[i] = ( ( (T)rand() / RAND_MAX ) * std::numeric_limits< T >::max() );
                mMemberS[i] = ( ( (S)rand() / RAND_MAX ) * std::numeric_limits< S >::max() );
                mMemberR[i] = ( ( (R)rand() / RAND_MAX ) * std::numeric_limits< R >::max() );
            }
        }

        void OnSerialise( Message &message )
        { 
            for ( uint32_t i=0; i < (its * 3); i+=3 )
            {
                message.Store( mMemberT[i], i, Flag );
                message.Store( mMemberS[i], i+1, Flag );
                message.Store( mMemberR[i], i+2, Flag );
            }
        }
